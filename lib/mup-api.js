  ...
  getSettings() {
    if (!this.settings) {
      ...
      try {
        ...
        console.log('Failed to load config file, reading settings...');
        console.log('METEOR_SETTINGS=' + process.env.METEOR_SETTINGS);
        if(process.env.METEOR_SETTINGS) {
          this.settings = JSON.parse(process.env.METEOR_SETTINGS);
        } else {
          console.error(`No Meteor Settings found, set 'METEOR_SETTINGS' environment variable or ensure a '%s' is a valid file.`, filePath);
          process.exit(1);
        }
      }
    }
