...
export function getLoadBalancingHosts(expandedServers, drainingHosts = []) {
  return Object.values(expandedServers).map(({ server }) => ({
    host: server.privateIp || server.host,
    params: drainingHosts.includes(server.host) ? 'down' : ''
  })
  );
}
